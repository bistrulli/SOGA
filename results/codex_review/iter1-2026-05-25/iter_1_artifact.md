...
3. **Symmetry reduction (H4)**: for fixed output cell `(r,s)`, only faults in input column `j=s` shift the mean; faults in `j≠s` leave `D[r,s]` distribution unchanged. Per-output-cell distinct cases collapse from 5121 to **26** (5 classes × 5 row-equivalence groups + baseline). Confirmed analytically by `numerical-stability-expert`.

### Fault model: 5-class bit-class mixture (moment-matching exact)

| Class | Bits | P(c) | Shift formula | Random-input handling |
|---|---|---|---|---|
| SIGN | 31 | 1/32 | δ = −2·v | exact (linear in v) |
| HIGH-EXP | 27-30 | 4/32 | δ = v·(2ᵏ−1), k∈{16,32,64,128} | exact (linear in v); OTR-prone via overflow |
| LOW-EXP | 23-26 | 4/32 | δ = v·(2ᵏ−1), k∈{1,2,4,8} | exact (linear in v) |
| HIGH-MANTISSA | 16-22 | 7/32 | δ = ±v·2ᵖ, p∈[−7,−1] | exact mixture (sign × p discrete) |
| LOW-MANTISSA | 0-15 | 16/32 | δ = ±v·2ᵖ, p∈[−23,−8] | exact mixture |

**Key finding from `probabilistic-programming-expert`**: moment-matching is **exact** for all 5 classes because the shift is always linear (or scalar-multiplicative) in the cell value `v`. Mantissa classes have `E[δ|v]=0` by sign symmetry, `Var[δ|v] = v² · E[2^(2p)]` over p uniform — both analytically computable.
...
for p in linspace(0, 1, 21):                                # bimodal Bernoulli
    ...
    if p in {0.0, 0.5, 1.0}:                                # MC validate
...
- [ ] **[0.3]** [iter:1] [agent:soga-internal-expert] [area:experiments] Verify `programs/Example/lishan_2mm_32x32.soga` runs end-to-end on the new branch; capture baseline `(M_D, U_D, V_D)` for the A used in the showcase. Store in `experiments/.../results/baseline_DUV.npz`.
...
- [ ] **[1.1]** [iter:1] [agent:gaussian-mixture-expert] [area:experiments/lib/fault_model.py] Implement `FaultClass` enum (SIGN, HIGH_EXP, LOW_EXP, HIGH_MANTISSA, LOW_MANTISSA) with `(probability, shift_formula, shift_variance)` per class. Use exact formulas from `gaussian-mixture-expert` memo (Q4).
...
- [ ] **[1.4]** [iter:1] [agent:test-engineer] [area:tests/test_fault_model.py] Unit tests on 5 mandatory edge cases (per H5):
  - `v=0` → all MSK
  - `v=1e-30` → mostly SDC (high-exp dominates)
  - `p_fault=0` → all MSK
  - `p_fault=1, v=1.0` → MSK ≥ P(LOW-MANTISSA)
  - `v=1e38` → OTR ≥ P(HIGH-EXP)
...
- [ ] **[3.2]** [iter:2] [agent:gaussian-mixture-expert] [area:experiments/predict_resilience_soga.py] Implement `compute_per_cell_SDC(M_D, U_D, V_D, A, v, eps, p_fault) -> {(r,s): (P_MSK, P_SDC, P_OTR)}` per the aggregate formula (Q6 of gm-expert memo). Use H4 symmetry reduction: per output cell only j=s faults shift the mean.
...
- [ ] **[3.5]** [iter:2] [agent:test-engineer] [area:tests/test_predict_soga.py] Validation: K=26 reduction must match K=5121 expansion on a small case (e.g., m=n=4). Run both, compare per-cell `P_SDC` element-wise (max |Δ| < 1e-10). This certifies the symmetry reduction.
...
- [ ] **[4.3]** [iter:3] [area:tests/test_step1_acceptance.py] Acceptance gate: assert `pearson(MC, SOGA) > 0.95` per curve, `max_rel_err < 10%` on MSK/SDC, `max_abs_err < 5%` on OTR.
...
- [ ] **[5.1]** [iter:3] [agent:gaussian-mixture-expert] [area:experiments/lib/bimodal_prior.py] Implement `bimodal_to_matrix_gaussian(p, V_low, V_high, m, n) -> (M_B, U_B, V_B)` using moment-matching:
  ...
  - Validity flag: emit warning when `p < 0.1 or p > 0.9` (Gaussian approximation degrades, per gm-expert Q5).
...
- [ ] **[5.4]** [iter:3] [area:experiments/simulate_fi_mc.py] Extend MC: sample `B_ij ~ Bernoulli(p)·V_high + (1-Bernoulli(p))·V_low` per cell, then apply bit-flip FI as in M2. Run on 3 representative points (p=0.0, p_critical, p=1.0) with 1000 samples each.
...
- M3 (SOGA prediction): 3 days — medium risk, K=26 reduction needs validation
...
Uncertainty flags:
- **U1**: K=26 symmetry reduction (M3.5) — must be confirmed; if wrong, fall back to K=5121 (slower but no semantic difference)
...
|----|------|----------|------------|
| R-LR1 | K=26 symmetry reduction wrong → silent error in Step 1 numbers | HIGH | M3.5 cross-validation against K=5121 mandatory before M4 |
| R-LR2 | Bimodal Bernoulli Gaussian approx breaks for p∈{0, 1} extremes | MEDIUM | M5.1 emit warning; MC validation at boundary points; consider 2-component GM fallback |
| R-LR3 | High-exp overflow handling (H1) misclassifies near-boundary cases | MEDIUM | M1.2 log-space check + M1.4 edge case `v=1e38` |
| R-LR4 | Pearson correlation < 0.95 in Step 1 → replica fails | MEDIUM | If <0.95, diagnose: (a) fault model approximation gap, (b) statistical noise (sample 10000 instead of 1000); accept down to 0.85 with documented gap |
...
| R-LR7 | scipy.stats.norm.logsf returns -inf for extreme z → P_SDC = 0 wrong | MEDIUM | M3.3 cap at -750 (below scipy underflow); test at z=±50 |
