...
- [ ] **[R0.1]** [iter:5] [area:experiments/lib] Write `lib/DESIGN_BIT_EXACT.md` design doc: rationale, IEEE 754 references, the 3 specialist-found bug fixes (NaN/Inf guard, float32-cast delta, native endianness), comparison vs 5-class, scope.
...
- [ ] **[R2.5]** [iter:5] [area:scripts] Add `--mode {bit_exact, 5_class}` flag to `run_soga_step1.py`, `run_step3.py`, `run_sweep.py`. Default `bit_exact`. Dispatch via mode-conditional import (soga-internal Q6).
...
- [ ] Step 1 Pearson > 0.95 between SOGA bit-exact and MC per (MSK, SDC) curve
...
| R-BE2 | NaN/Inf input not guarded → propagation of NaN deltas | MEDIUM | Numerical-stability Bug A fix: explicit `not np.isfinite(v)` guard at entry; R1.4 NaN edge case test |
